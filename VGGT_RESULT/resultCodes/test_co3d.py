...
def setup_args():
    ...
    parser.add_argument('--co3d_dir', type=str, required=False,default="/home/knuvi/dataset/Blind_images_v2", help='Path to CO3D dataset')
    parser.add_argument('--co3d_anno_dir', type=str, required=False,default="/home/knuvi/dataset/Blind_images_v2", help='Path to CO3D annotations')
    parser.add_argument('--seed', type=int, default=0, help='Random seed for reproducibility')
    parser.add_argument('--model_path', type=str, required=False, default="/home/knuvi/model_tracker_fixed_e20.pt",help='Path to the VGGT model checkpoint')
...
def process_sequence(model, seq_name, seq_data, category, co3d_dir, min_num_images, num_frames, use_ba, device, dtype, QP):
    """
    ...
    for data in seq_data:
        ...
        gt_extrinsic123.append(extri_opencv)
        #i+=1
        #print(i ,":", extri_opencv)
        #input()
    print("extri end")
    #input()
    ...
    '''
    num_frames = 10
    ids = np.linspace(0, len(metadata) - 1, num_frames, dtype=int)
    print("Image ids:", ids)
    print("Selected frames:")
    for i in ids:
        print(f"  {i}: {metadata[i]['filepath']}")

    image_names = [os.path.join(co3d_dir, metadata[i]["filepath"]) for i in ids]
    print("images_names:", image_names)
    input()
    '''
    num_frames = 10
    #QP = 27
    #frame_nums = [extract_frame_number(m["filepath"]) for m in metadata]

    if category=="laptop":
        ids = np.linspace(3, len(metadata) - 1, num_frames, dtype=int)
    else :
        ids = np.linspace(0, len(metadata) - 1, num_frames, dtype=int)
    ...
    if QP==0 :
        for i in ids:
            old_path = metadata[i]["filepath"]  # 예: backpack/159_17477_32932/images/frame000200.jpg
            frame_num = os.path.basename(old_path).split('.')[0].replace("frame", "")  # "000200"
            seq_name = old_path.split("/")[1]  # "159_17477_32932"

            new_path = f"/home/knuvi/dataset/Blind_images_v2/{category}/{seq_name}/images/frame{int(frame_num):06d}.jpg"
        ...
        for i in ids:
            ...
            new_path = f"/home/knuvi/dataset/Blind_images_v2/avc/{category}/{seq_name}/images_qp{QP}/frame{int(frame_num):06d}_AVCRA_{QP}.png"
            image_names.append(new_path)

    print("Compressed image paths:")
...
def main():
    ...
    SEEN_CATEGORIES = ["backpack", "ball", "book", "chair", "handbag", "laptop", "plant", "teddybear", "vase"]
    #SEEN_CATEGORIES = ["laptop"]
    if args.debug:
    ...
    for QP in QP_list:
        ...
        for category in SEEN_CATEGORIES:
            ...
            annotation_file = os.path.join(args.co3d_anno_dir, f"{category}_test.json")
            ...
            for seq_name in seq_names:
                ...
                seq_rError, seq_tError = process_sequence(
                    model, seq_name, seq_data, category,
                    args.co3d_dir, args.min_num_images, args.num_frames,
                    args.use_ba, device, dtype, QP
                )
            ...
            print("="*80)
            print(f"AUC of {category} (QP={QP}): {Auc_30:.4f} (30), {Auc_15:.4f} (15), {Auc_5:.4f} (5), {Auc_3:.4f} (3)")
            print("="*80)

        # Summary per QP
        print("\nSummary of AUC results (QP={}):".format(QP))
        print("-"*50)
        for category in sorted(per_category_results.keys()):
            print(f"{category:<15}: {per_category_results[category]['Auc_30']:.4f} (30), "
                  f"{per_category_results[category]['Auc_15']:.4f} (15), "
                  f"{per_category_results[category]['Auc_5']:.4f} (5), "
                  f"{per_category_results[category]['Auc_3']:.4f} (3)")

        csv_path = os.path.join(os.path.dirname(args.model_path), f"auc123_summary_qp{QP}.csv")
        with open(csv_path, mode="w", newline="") as f:
            import csv
            writer = csv.writer(f)
            writer.writerow(["Category", "AUC@30", "AUC@15", "AUC@5", "AUC@3"])
            for category in sorted(per_category_results.keys()):
                writer.writerow([
                    category,
                    f"{per_category_results[category]['Auc_30']:.4f}",
                    f"{per_category_results[category]['Auc_15']:.4f}",
                    f"{per_category_results[category]['Auc_5']:.4f}",
                    f"{per_category_results[category]['Auc_3']:.4f}"
                ])

        print(f"\n✅ Saved AUC summary for QP={QP} → {csv_path}")
    """
    per_category_results = {}
    ...
    for category in SEEN_CATEGORIES:
        ...
        for seq_name in seq_names:
            ...
            seq_rError, seq_tError = process_sequence(
                ...
                args.min_num_images, args.num_frames, args.use_ba, device, dtype, QP
            )
    ...
    #=== CSV 저장 부분 추가 ===
    csv_path = os.path.join(os.path.dirname(args.model_path), f"auc_summary{QP}.csv")
    with open(csv_path, mode="w", newline="") as f:
        writer = csv.writer(f)
        writer.writerow(["Category", "AUC@30", "AUC@15", "AUC@5", "AUC@3"])
    ...
    print(f"\n✅ Saved per-category AUC summary to: {csv_path}")
"""
